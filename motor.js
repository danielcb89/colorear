...
// VARIABLES DEL MODO ARCOÍRIS LIBRE CORREGIDAS
let modoArcoiris = false;
let tonoArcoiris = 0;
const velocidadArcoiris = 5; // 👈 AHORA SÍ FUNCIONA: Sube a 15 si lo quieres aún más rápido o baja a 6 si va muy acelerado
...
portfolioTitle.innerText = "🖼 " + categoria;
portfolioContainer.innerHTML = "";
backButtonContainer.innerHTML = "";
const btnBack = document.createElement('button');
btnBack.className = 'btn-back-cats';
btnBack.innerText = "⬅ Categorías";
btnBack.onclick = () => {
...
const btnDestino = document.getElementById("tool-" + modo);
if (btnDestino) btnDestino.classList.add('active');

if (modo === 'arcoiris') {
activarBucleArcoiris();
document.querySelectorAll('.color-bubble').forEach(b => b.classList.remove('selected'));
} else {
desactivarBucleArcoiris();
if (modo !== 'borrador') {
...
function deseleccionarArcoirisNativo() {
desactivarBucleArcoiris();
}
...
// SOLUCIÓN TÉCNICA: El incremento ahora multiplica de forma real usando la variable global
function activarBucleArcoiris() {
modoArcoiris = true;
if (!temporizadorArcoiris) {
temporizadorArcoiris = setInterval(() => {
tonoArcoiris = (tonoArcoiris + velocidadArcoiris) % 360; // 👈 Aquí estaba el fallo, ahora suma los grados configurados
colorActual = `hsl(${tonoArcoiris}, 100%, 50%)`;
}, 25);
}
}

function desactivarBucleArcoiris() {
modoArcoiris = false;
if (temporizadorArcoiris) {
clearInterval(temporizadorArcoiris);
temporizadorArcoiris = null;
}
}
