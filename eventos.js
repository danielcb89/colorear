...
const mapaDeSimbolos = {
    sol: '☀️', nube: '☁️', lluvia: '🌧️', luna: '🌙', estrella: '⭐',
    destello: '✨', rayo: '⚡', arcoiris: '🌈', arbol: '🌳', pino: '🌲',
    matorral: '🌿', flor: '🌸', tulipan: '🌷', hierba: '🌱', seta: '🍄',
    montana: '⛰️', ola: '🌊', pez: '🐟', cangrejo: '🦀', estrellaMar: '⭐',
    caracola: '🐚', burbuja: '🫧', gota: '💧', ancla: '⚓', corazon: '❤️',
    mariposa: '🦋', sonrisa: '🙂', corona: '👑', globo: '🎈', nieve: '❄️',
...
let formaColocadaInicialmente = false; // NUEVO: Controla si es la primera vez que se toca el papel
...
function iniciarNuevaFormaFlotante(id) {
    limpiarLienzoTemporal();
    // Ya no forzamos el centro de la pantalla de inicio
    formaEscala = 1.0; formaAngulo = 0; baseEscala = 1.0; baseAngulo = 0;
    formaColocadaInicialmente = false; // Esperando el primer toque real en el dibujo
}
...
canvas.parentElement.addEventListener('touchstart', (e) => {
    ...
    if (e.touches.length === 1) {
        pintando = true;
        // Determinamos la posición del dedo
        ...
        // Si es el primer toque en el folio, activamos la visualización en ese sitio exacto
...
canvas.addEventListener('pointermove', (e) => { if (!pintando || modoActual === 'relleno' || formaSeleccionada) return; const pos = obtenerPos(e); ctx.lineTo(pos.x, pos.y); ctx.stroke(); });
